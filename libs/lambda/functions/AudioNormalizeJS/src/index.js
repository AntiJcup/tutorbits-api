...
exports.handler = function (eventObject, context) {
	...
	return s3Util.downloadFileFromS3(inputBucket, inputFile, localInputFile)
		.then(() => childProcessPromise.spawn(
			'/opt/bin/ffmpeg',
			['-loglevel', 'error', '-y', '-i', localInputFile, '-vcodec', 'libx264', '-vprofile', 'high', '-preset', 'ultrafast', '-b:v', '500k', '-maxrate', '500k', '-bufsize', '3000m', '-vf', 'scale=-1:480', '-codec:a', 'aac', '-strict', 'experimental', '-b:a', '128k', '-af', 'highpass=200, lowpass=1500, loudnorm=I=-35:TP=-1.5:LRA=20', '-threads', '0', localOutputFile],
